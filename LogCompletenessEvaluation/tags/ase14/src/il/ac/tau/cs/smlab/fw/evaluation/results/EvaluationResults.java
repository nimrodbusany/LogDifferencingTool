...
public class EvaluationResults {
	...
	// a threshold for a log to be considered complete
	public static final double COMPLETENESS_THRESHOLD = 1;
	...
	public String generateToString(boolean showIntermediateConfidence) {
		...
		for (SingleModelEvaluationResults mr : models) {
			...
			sb.append("\n");
			sb.append("redundancy (mean,sd): " + avg(redundancy) + "," + sd(redundancy));
			sb.append("\n");
		...
		sb.append("\n");
		sb.append("redundancy (mean,sd): " + avg(allRedundancy) + "," + sd(allRedundancy));
		sb.append("\n");
		sb.append("\n");
	...
	protected int countCompleteLogs(List<Double> completeness) {
		int sum = 0;
		for (double c: completeness) {
			if (c >= COMPLETENESS_THRESHOLD) {
				++sum;
			}
		}
		return sum;
	...
	protected double maxIncorrectSeqs(List<Double> incorrect, List<Double> completeness) {
		double max = 0;
		for (int i = 0 ; i<completeness.size();++i) {
			if (completeness.get(i) < COMPLETENESS_THRESHOLD) {
	...
	public List<String[]> getResultEntries() {
		...
		for (SingleModelEvaluationResults mr : models) {
			...
			entry[headline.indexOf("redundancy (mean)/redundancy (sd)")] = String.format("%.2f",avg(redundancy)) + "/" + String.format("%.2f",sd(redundancy));
