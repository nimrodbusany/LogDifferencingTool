...
public class LPValidation {
	...
	public static final String DOT_LOCATION = "c:\\graphviz-2.36_\\release\\bin\\dot.exe";

	@Test
	public void validationForKTails() throws SpecMiningAlgorithmException, InvalidModelException, IOException {
		KTailsAlgorithm algorithm = new KTailsAlgorithm(new KTailsInputParams(K));
	//	algorithm.addLogProperty(new TerminalEventProperty());
		// validate each model
		List<FSAInputModel> models = EvaluationModelsManager.getTestingModels();
	...
	@Test
	public void validationForSynoptic() throws Exception {
	...
	private SynopticInputParams getSynopticInputParams(String log) {
		return new SynopticInputParams("validation/" + log, "--", "(?<TYPE>.*)", null,DOT_LOCATION);
	}
