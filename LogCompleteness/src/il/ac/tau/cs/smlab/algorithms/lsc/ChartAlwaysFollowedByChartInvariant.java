...
public class ChartAlwaysFollowedByChartInvariant implements ITemporalInvariant{
    ...
    public ChartAlwaysFollowedByChartInvariant(List<EventType> preChart, List<EventType> mainChart) {
	...
	public String getLTLString() {
		...
		String antecedent = sb.substring(4);
		sb.setLength(sb.length() - otherEvents.length() - 5); // remove last otherEvents
		sb.append(br);
		otherEvents = otherEvents.substring(0, otherEvents.length() - 1);
/*		otherEvents = otherEvents + "&& ";
		for (String m:main) {
		otherEvents = otherEvents + "!(did("+m+")) && ";
		}
		otherEvents = otherEvents.substring(0, otherEvents.length() - 3); */
		otherEvents = otherEvents + ")";
		sb.append(" -> ");
		sb.append("(");
		sb.append(antecedent);
		for (String m:main) {
			sb.append(" U(did("+m+") && X("+otherEvents);
			br = br + "))";
		}
		sb.setLength(sb.length() - otherEvents.length() - 5); // remove last otherEvents
		sb.append(br);
		sb.append(")");
		sb.append(")");
		return "(<>(did(TERMINAL))) ->" + sb.toString();
	}

	@Override
	public String getLongName() {
		return "ChartAlwaysFollowedByChart";
	}
