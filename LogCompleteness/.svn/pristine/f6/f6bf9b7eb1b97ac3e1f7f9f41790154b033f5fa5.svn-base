...
public class KPermutations<T> {
  ...
  private List<List<T>> processLargerSubsets( List<List<T>> result, List<T> set, List<T> subset, int subsetSize, int nextIndex ) {
    if ( subsetSize == subset.size() ) {
      ...
      for ( int j = nextIndex; j < set.size(); j++ ) {
        ...
        processLargerSubsets( result, set, subset, subsetSize + 1, j );
      }
    }
    return result;
  }
  ...
  private static <T> ImmutableList<T> removeDuplicate(final Collection<T> collection) {
	  return ImmutableSet.copyOf(collection).asList();
	}
}